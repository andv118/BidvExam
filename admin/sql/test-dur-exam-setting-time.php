<?php
	session_start();
	if (isset($_POST['mt'])&&isset($_POST['time'])){
		include("../../config.php");
		if (isset($_POST['hv'])&&$_POST['hv']!=""){ // Đặt lại bài làm cho một học viên theo số báo danh
			...
			mysqli_query($connect, "update diem set diem=0, thoigianketthuc='', timeconlai=".($_POST['time']*60)." where sbd='".$_POST['hv']."' and mamodun='".$_POST['mt']."'");
			...
			// $t=mysqli_query($connect, "select tgthi from thoigianthi where mamodun='".$_POST['mt']."'");
			// $r=mysqli_fetch_array($t, MYSQLI_ASSOC);
			mysqli_query($connect, "update diem, hocvien, phongthi set diem=0, thoigianketthuc='', timeconlai=".($_POST['time']*60)." where diem.sbd=hocvien.sbd and hocvien.mapt=phongthi.mapt and phongthi.mapt='".$_POST['pt']."' and diem.mamodun='".$_POST['mt']."'");
