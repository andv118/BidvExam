...
<body>
                ...
                <form class='form-material form-horizontal' style="width: 100%;">
                    <div class="form-body">
                        <div class="form-group">
                            <label class="col-md-12" for="example-text">Mã nội dung thi</label>
                            <div class="col-md-12">
                                <input type="text" id="mabode" class="form-control" value="<?php echo $_GET['id_bode']; ?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-12" for="example-text">Tên nội dung</label>
                            <div class="col-md-12">
                                <input type="text" id="tenbode" class="form-control" value="<?php echo $_GET['id_tenbode']; ?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-12" for="example-text">Điểm tối đa</label>
                            <div class="col-md-12">
                                <input type="number" id="diemmax" class="form-control" value="<?php echo $_GET['id_diemmax']; ?>">
                            </div>
                        </div>
                    </div>
                </form>
    ...
    <script type="text/javascript">
        $(document).ready(function(e){
            $("#sb").click(function(e){
                if (confirm("Sửa nội dung thi?")){
                    $(".preloader").show();

                    var mabode=$("#mabode").val();
                    var tenbode=$("#tenbode").val();
                    var diemmax=$("#diemmax").val();
                    var modun="<?php if (isset($_GET['id_mamodun'])) echo $_GET['id_mamodun']; else echo ""; ?>";

                    $.ajax({
                        type:'post',
                        url:'sql/edit-sub-subject.php?id=<?php if (isset($_GET['id_bode'])) echo $_GET['id_bode']; else echo ""; ?>',
