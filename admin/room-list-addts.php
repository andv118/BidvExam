...
<body>
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row bg-title" style="margin-bottom: 0.5em; padding: 0.5em;">
                <div class="col-md-12">
                    <h4>QUẢN LÝ HỌC VIÊN PHÒNG:<span style="color: red;"> <?php echo $_GET['cathi']." - ".$_GET['tenpt']; ?></span></h4>
                </div>
            </div>
            <?php
            	include("../config.php");
	            			...
	            			<?php
	            				while ($r=mysqli_fetch_array($d, MYSQLI_ASSOC)){
	            					echo "<tr>";
	            						...
	            						echo "<td>".$r['tendv']."</td>";
	            						echo "<td class='text-nowrap'><a href='room-list-addts-edit.php?id_edit=".$r['sbd']."' data-toggle='tooltip' data-original-title='Sửa thông tin'><i class='fa fa-edit text-inverse m-r-10'></i></a><a data-toggle='tooltip' data-original-title='Xóa thí sinh' onClick='deleteRow(\"".$r['sbd']."\");'><i class='fa fa-close text-danger' style='cursor:pointer;'></i> </a> </td>";
	            					echo "</tr>";
	            				}
	            			?>
	            		</tbody>
    ...
    <script type="text/javascript">
        ...
        function deleteRow(idM){
            if (confirm("Bạn chắc chắn xóa thí sinh này? Sau khi xác nhận xóa, tất cả dữ liệu liên quan đến thí sinh sẽ bị xóa và không thể phục hồi!")){
                // alert(idM);
                $(".preloader").show();
                $.ajax({
                    type: 'post',
                    url: 'sql/room-user-del.php',
                    data: {d1:idM},
                    success:function(e){
                        if (e=="true"){
            ...
            $("#upload-btn").click(function(e){
                ...
                if (v=="") alert("Bạn chưa chọn file");
                    ...
                    if (temp!="xslx"&&temp!="slx") alert("Bạn chỉ được tải lên file excel!");
                        ...
                        ajax.open("post","sql/hv-upload-fexcel.php?id=<?php if (isset($_GET['idPt'])) echo $_GET['idPt']; else echo ""; ?>");
                        ajax.send(upload);
